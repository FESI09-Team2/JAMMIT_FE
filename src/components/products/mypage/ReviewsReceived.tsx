...
export default function ReviewsReceived() {
  ...
  const dummyData = {
    totalReviews: 10,
    goodWithMusicPercentage: 80,
    practiceHelpedPercentage: 65,
    keepingPromisesPercentage: 70,
    goodWithOthersPercentage: 60,
    managingWellPercentage: 75,
    sharesPracticeResourcesPercentage: 55,
    helpfulPercentage: 85,
    goodLearnerPercentage: 50,
  };
  return (
    <div className="pc:max-w-[84rem] mx-auto mt-5 flex items-start gap-5">
      <div className="w-[23rem] rounded-lg bg-[#28282a] px-[2.375rem] pt-[3.75rem] pb-[2.375rem]">
        <div className="mx-auto h-[18.5rem] w-[18.5rem]">
          {dummyData.totalReviews > 0 && (
            <Radar
              data={{
                ...
                datasets: [
                  {
                    ...
                    data: REVIEW_METRICS.map(
                      (data) =>
                        dummyData[data.percentageKey as keyof typeof dummyData],
                    ),
      ...
      <ul className="flex flex-auto flex-col gap-5">
        {mockReviews.map((item: ReviewPros) => (
          <li
